...
Account health is separate from candidate identity. `ProviderAccountSelector`
filters child-owned, restart-safe account health and cooldown state while preserving the
existing fair account order; it deliberately does not create application-owned
provider-account persistence. Account-local authentication/rate-limit failures
can move to another account; media/provider temporary failures may move to the
next candidate. Processing and Telegram delivery failures never trigger
provider fallback. Every attempt is auditable in `download_provider_attempts`,
while all attempts remain inside one Stage 21 `DownloadJob` lifecycle.
