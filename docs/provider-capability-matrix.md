...
| Qobuz | Required | Active pool entry and token structure are observable; application authorization now validates an authenticated catalog operation after child-owned login | Initialization performs the pinned connectivity/login path; health performs a bounded read-only authenticated catalog probe | Missing/failed runtime → `AUTH_REQUIRED`; active but unverified → `AUTH_REQUIRED / SESSION_INVALID`; verified session → `READY` | Provider-level `READY` reflects a verified runtime session only; `check_source()` remains authoritative for each TrackSource and entitlement. |
